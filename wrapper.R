...
vector.error <- for(i in 1:nrow(params)){
  tryCatch(SimulateCommnunity(params$comm.size[i],params$comm.spp[i],params$comm.timesteps[i],params$comm.migrate[i],params$comm.env[i],params$comm.abund[i],params$comm.stoch[i],params$comm.speciate[i],params$intra.birth[i],params$intra.death[i],params$intra.steps[i],params$seq.birth[i],params$seq.death[i],params$seq.steps[i]), error= function(cond){traceback()})
}

#write.table(sim.Results,"~/OTU_Phylogeny/simResults.csv") 
