...
add.branch <- function(tree,birth,death,steps,type=c("pops","seq.err")){
  ...
  type <- match.arg(type)
  # If argument `tree` is without branches (i.e. NULL), return a tree that is NULL
  if(is.null(tree)){
    return(tree)
    # Otherwise, add branches to it
  }else{
    ...
    for(i in 1:tips){
      ...
      for (j in 1:new.tips){
        ...
        # The `type` argument denotes whether to add a "p" or an "e" to the new tips
