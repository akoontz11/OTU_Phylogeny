...
# 1. Change in site "ranking" (of least diverse -> most diverse, say) versus changes in delta value
  ...
  # c. Plot those averages over change in delta
        # This will only be for a single site, though. For multiple sites, you'd have to use multiple lines on the graph
...
for(i in 1:length(deltas)-1){
  ...
  shifts <- match(baseline, comparison)
...
plot(meanRankChanges ~ deltaDifferences)
